...
using PoeOverlay.Framework.InputHooks;
...
namespace PoeOverlay.Hud.Tooltip {
    public class ItemTooltipPlugin : Plugin {
                ...
                lock (Locker) {
                    ...
                    if (position > checkRadius/2) {
                        ...
                        if (!string.Equals(clipboard, bufferClipBoard)) {
                            if (string.Equals(clipboard, bufferClipBoard)) return filter;
        ...
        private void DrawTooltip(FilterResult curFilter) {
            ...
            if (x - width < 0) { x += (width - x + curFilter.BorderWidth); }
            if (y - height < 0) { y += (height - y + curFilter.BorderWidth); }

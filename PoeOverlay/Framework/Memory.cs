...
using System.Text;
using PoeOverlay.Framework.Enums;
using PoeOverlay.Model;
using PoeOverlay.Poe;

namespace PoeOverlay.Framework {
    public class Memory : IDisposable {
        public readonly Int64 AddressOfProcess;
        ...
        public bool IsInvalid() { return Process.HasExited || closed; }

        public int ReadInt(int addr) { return BitConverter.ToInt32(ReadMem(addr, 4), 0); }
        ...
        public float ReadFloat(int addr) { return BitConverter.ToSingle(ReadMem(addr, 4), 0); }

        public long ReadLong(int addr) { return BitConverter.ToInt64(ReadMem(addr, 8), 0); }

        public uint ReadUInt(int addr) { return BitConverter.ToUInt32(ReadMem(addr, 4), 0); }

        public short ReadShort(int addr) { return BitConverter.ToInt16(ReadMem(addr, 2), 0); }
        ...
        public byte ReadByte(int addr) { return ReadBytes(addr, 1).FirstOrDefault(); }

        public byte[] ReadBytes(Int64 addr, int length) { return ReadMem(addr, length); }
        ...
        private byte[] ReadMem(Int64 addr, int size) {
            ...
            for (int iPattern = 0; iPattern < patterns.Length; iPattern++) {
                ...
                for (int offset = 0; offset < exeImage.Length - patternLength; offset += 4) {
                    if (CompareData(pattern, exeImage, offset)) {
                        ...
                        Console.WriteLine("Pattern " + iPattern + " is found at " + (AddressOfProcess + offset).ToString("X"));
                        break;
                    }
                }
            }
        ...
        private bool CompareData(Pattern pattern, byte[] data, int offset) {
            for (int i = 0; i < pattern.Bytes.Length; i++) {
                if (pattern.Mask[i] == 'x' && pattern.Bytes[i] != data[offset + i]) return false;
            }
            return true;
        }
    }

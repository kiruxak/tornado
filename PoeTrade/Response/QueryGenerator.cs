...
namespace PoeTrade.Response {
    public class QueryGenerator {
        private readonly Dictionary<string, string> generalAffixes = new Dictionary<string, string>
                                            { {"league", "Standard"}, {"type",""}, {"base",""}, {"name",""}, {"dmg_min",""}, {"dmg_max",""},
        ...
        public string GenerateQuery(List<ITradeAffix> itemAffixes, Dictionary<string, string> generalParams) {
        ...
        private void createQueryParams(List<ITradeAffix> itemAffixes) {
            itemAffixes.Each(a => {
                ...
                affixes.Add(prefix + expression + "&mod_min=" + string.Format(new System.Globalization.CultureInfo("en-US"), "{0:F2}", a.Value * 0.95) + "&mod_max=");
            });
        }

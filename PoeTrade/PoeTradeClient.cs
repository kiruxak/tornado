...
namespace PoeTrade {
    public class PoeTradeClient {
        ...
        public async Task<string> GetData(List<ITradeAffix> affixes, Dictionary<string, string> gerenalParams) {
        ...
        public async Task<List<PoeItemData>> GetPrice(List<ITradeAffix> affixes, Dictionary<string, string> gerenalParams) {
            var client = new PoeTradeClient();
            var data = await client.GetData(affixes, gerenalParams);
            return PoeItemData.Parse(data);
        }

        public async void OpenPoeTrade(List<ITradeAffix> affixes, Dictionary<string, string> gerenalParams) {
            var client = CreateHttpClient();
            var x = new QueryGenerator().GenerateQuery(affixes, gerenalParams);
            var content = new StringContent(x, Encoding.UTF8, "application/json");

...
namespace PoeParser.Entities {
    public static class ItemTypes {
        ...
        private static Core GetMagicItemBase(string magicItemBase) {
            foreach (KeyValuePair<string, WeaponBase> pair in PoeData.Weapons) { if (magicItemBase.Contains(pair.Key)) { return new Core(pair.Value.Type, pair.Value); } }
            foreach (var body in PoeData.Armors.Values) { if (magicItemBase.Contains(body.Name)) { return new Core(ItemType.BodyArmour, PoeData.Armors[body.Name]); } }
            foreach (var boots in PoeData.Boots.Values) { if (magicItemBase.Contains(boots.Name)) {  return new Core(ItemType.Boots, PoeData.Boots[boots.Name]); } }
            foreach (var gloves in PoeData.Gloves.Values) { if (magicItemBase.Contains(gloves.Name)) { return new Core(ItemType.Gloves, PoeData.Gloves[gloves.Name]); } }
            foreach (var helm in PoeData.Helmets.Values) { if (magicItemBase.Contains(helm.Name)) { return new Core(ItemType.Helmet, PoeData.Helmets[helm.Name]); } }
            foreach (var amulet in Amulets) { if (magicItemBase.Contains(amulet)) { return new Core(ItemType.Amulet); } }
            foreach (var ring in Rings) { if (magicItemBase.Contains(ring)) { return new Core(ItemType.Ring); } }
            foreach (var belt in Belts) { if (magicItemBase.Contains(belt)) { return new Core(ItemType.Belt); } }
            foreach (var shield in PoeData.Shields.Values) { if (magicItemBase.Contains(shield.Name)) { return new Core(ItemType.Shield, PoeData.Shields[shield.Name]); } }
            foreach (var jewel in Jewels) { if (magicItemBase.Contains(jewel)) { return new Core(ItemType.Jewel); } }
            if (magicItemBase.IndexOf("Flask") > 0) { return new Core(ItemType.Flask); }
            return new Core(ItemType.Unknown);
        }
        ...
        public static List<string> Maps = new List<string>() {
            "Arcade", "Crystal Ore", "Desert", "Jungle Valley", //t1
            "Beach", "Factory", "Ghetto", "Oasis", //t2
            "Arid Lake", "Cavern", "Channel", "Grotto", "Marshes", "Sewer", "Vaal Pyramid", //t3
            "Academy", "Acid Lakes", "Dungeon", "Graveyard", "Phantasmagoria", "Villa", "Waste Pool", //t4
            "Burial Chambers", "Bunes", "Mesa", "Peninsula", "Pit", "Spider Lair", "Tower", //t5
            "Canyon", "Quarry", "Racecourse", "Ramparts", "Spider Forest", "Strand", "Thicket", "Vaal City", "Wharf", //t6
            "Archnid Tomb", "Armoury", "Ashen Wood", "Castle Ruins", "Catacombs", "Cells", "Mud Geyser", //t7
            "Arachnid Nest", "Arena", "Atoll", "Barrows", "Bog", "Cemetery", "Pier", "Shore", "Tropical Island", //t8
            "Coves", "Crypt", "Museum", "Orchard", "Overgrown Shrine", "Promenade", "Reef", "Temple", //t9
            "Arsenal", "Colonnade", "Courtyard", "Malformation", "Quay", "Terrace", "Underground River",  //t10
            "Bazaar", "Chateau", "Excavation", "Precinct", "Torture Chamber", "Underground Sea", "Wasteland", //t11
            "Crematorium", "Estuary", "Ivory Temple", "Necropolis", "Plateau", "Residence", "Shipyard", "Vault",  //t12
            "Beacon", "Gorge", "High Gardens", "Lair", "Plaza", "Scriptorium", "Sulphur Wastes", "Waterways", //t13
            "Maze", "Mineral Pools", "Palace", "Shrine", "Springs", "Volcano", //t14
            "Abyss", "Colosseum", "Core", "Dark Forest", "Overgrown Ruin", //t15
            "Forge of the Phoenix", "Lair of the Hydra", "Maze of the Minotaur", "Pit of the Chimera", "Vaal Temple", //t16

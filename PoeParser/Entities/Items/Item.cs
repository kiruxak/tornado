...
namespace PoeParser.Entities {
    public abstract class Item : BaseItem {
        ...
        public Dictionary<string, IAffixValue> Affixes { get; set; } = new Dictionary<string, IAffixValue>();
        ...
        protected Item(string source, ItemRarity rarity, Core core, string name) : base(source, rarity, core.Type, name) {
            ...
            if (parts.Last().ContainsPattern("\r\nMirrored")) parts.RemoveAt(parts.Count - 1); // Mirrored
            if (parts.Last().ContainsPattern("\r\nCorrupted")) parts.RemoveAt(parts.Count - 1); // Corrupted
            if (parts.Last().ContainsPattern("Note:")) parts.RemoveAt(parts.Count - 1); // Note with b/o
            if (parts.Last().ContainsPattern("\r\nHas ")) parts.RemoveAt(parts.Count - 1); // Effect
        ...
        public override List<ColoredLine> GetTooltip(FilterResult filter) {
            ...
            foreach (KeyValuePair<FilterGroup, List<IAffixValue>> pair in groupMaps.OrderBy(g => g.Key.Order)) {
                ...
                pair.Value.OrderBy(a => a.RegexPattern).Each(a => {
            ...
            if (Config.Debug) {
                ...
                foreach (var a in filter.PossiblyToCraft.OrderBy(a => a.Type)) {
                    ...
                    if (tier!=null) lines.Add($"{a.Name}[{(a.Type == AffixType.Prefix ? "P" : "S")}]: {tier.MinValue}/{tier.MaxValue}", color);
                }
            }
            return lines;
        }

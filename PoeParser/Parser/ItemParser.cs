    ...
    public class ItemParser {
        ...
        public static FilterResult Parse(string source) {
            try {
                ...
                if (rarity == ItemRarity.Gem) { item = new GemItem(source, name); }

                if (cache.Contains(name)) return (FilterResult)cache[name];
                if (item != null && cache.Contains(item.CacheName)) return (FilterResult)cache[item.CacheName];

                Core core = ItemTypes.GetItemBase(lines);

                if (item == null) {
                    if (rarity == ItemRarity.Normal && core.Type == ItemType.Unknown) item = new NormalItem(source, name);
                    if (rarity == ItemRarity.DivinationCard) item = new DivinationCard(source, name);

                    if (rarity == ItemRarity.Unique) {
                        item = new UniqueItem(source, core, name, lines[2]);
                        if (item.Unindentified) item = null;
                    }
                    if (core.Type == ItemType.Jewel && rarity != ItemRarity.Unique) return null; //todo jewels atleast track life or es
                    if (rarity == ItemRarity.Rare) {
                        if (core.Type.IsWeapon()) item = new WeaponItem(source, rarity, core, name);
                        if (core.Type.IsJewelery() || core.Type == ItemType.Quiver) item = new JewelryItem(source, rarity, core, name);
                        if (core.Type.IsArmor()) item = new ArmourItem(source, rarity, core, name);
                    }
                }
                ...
                if (rarity == ItemRarity.Rare) { item.Color = f.Color; }

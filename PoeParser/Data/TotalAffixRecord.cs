...
namespace PoeParser.Data {
    public class TotalAffixRecord : IRecord {
        ...
        private static readonly Func<IAffixValue, string> SetAffixMinValue = (a) => a.MinValue.ToString(new CultureInfo("en-US"));
        private static readonly Func<IAffixValue, string> SetAffixMaxValue = (a) => a.MaxValue.ToString(new CultureInfo("en-US"));
        private static readonly Func<IAffixValue, string> SetAffixValue = (a) => a.Value.ToString(new CultureInfo("en-US"));
        ...
        private double getValue(Item item, Func<IAffixValue, string> setAffixValue) {
            string value = item.Affixes.Where(a => a.Value is AffixValue).Aggregate(MathExpression, (current, a) => current.Replace(a.Key, setAffixValue(a.Value).ToString(new CultureInfo("en-US"))));
